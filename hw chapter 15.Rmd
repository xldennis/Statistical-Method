...
plot(y ~ x, data.17, pch = 19, col = "gray40", 
     xlab = "The dollar increase in annual dues", ylab = "Fitted Value")
lines(xx, predict(logit.17, data.frame(x = xx), type = "resp"), lwd = 2, col='blue')
a= coef(logit.17)[1]
b= coef(logit.17)[2]
curve(exp(a+b*x)/(1+exp(a+b*x)), from = min(data.17$x), to = max(data.17$x), 
      col='green',add=T)
...
```{r, echo=T,message=FALSE,comment=NA}
cat("the estimated probability that association members will not renew
    their memeberhip if \n the dues are increased by $40 is", 
    predict(logit.17, type = "response",newdata = list(x=40)))
```
...
```{r, echo=T,message=FALSE,comment=NA}
cat("the estimated probability that clients aged 55 with a health awareness 
    index of 60 \n (fixed x3 = 1) is", 
    predict(logit.14, type = "response",newdata = list(x1=55,x2=60,x3=1)))
cat("the estimated probability that clients aged 55 with a health awareness 
    index of 60 \n (fixed x3 = 0) is", 
    predict(logit.14, type = "response",newdata = list(x1=55,x2=60,x3=2)))
```
